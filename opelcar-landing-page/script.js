...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (cotizacionForm && quoteModal) {
    ...
    cotizacionForm.addEventListener('submit', (e) => {
      ...
      // Mostrar toast informativo de envío
      showToast('Enviando cotización directamente al departamento administrativo...', 'info');
      ...
      .then(data => {
        ...
        if (data.success) {
          // Inyectar datos retornados por el servidor en la ficha
          ...
          showToast('¡Cotización enviada exitosamente al servidor!', 'success');
        } else {
      ...
      .catch(error => {
        ...
        // Fallback: Mostrar la ficha y lanzar mailto tradicional tras breve retardo
        ...
        const subjectText = `Nueva Solicitud de Cotización OP-${randNum} - ${nombre}`;
        const emailBodyText = `==================================================
SOLICITUD DE COTIZACIÓN - OPELCAR S.A.S
==================================================
No. Documento: OP-${randNum}
Fecha: ${fechaStr}
